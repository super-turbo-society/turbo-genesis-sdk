...
use syn::{
    ...
    parse_macro_input, Item, ItemEnum, ItemMod, ItemStruct, LitStr,
};
...
pub fn program(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if let Some((_, items)) = &mut module.content {
                ...
                | Item::Enum(ItemEnum { ident, attrs, .. }) => {
                    ...
                    for attr in attrs {
                            ...
                            .map_or(false, |seg| seg.ident == COMMAND_ATTR_IDENT)
                        {
                            ...
                            let handler_export =
                                format!("turbo_program:command_handler/{}/{}", program_id, name);
                            let handler_extern =
                                format_ident!("command_handler_{}_{}", program_name, name);
                            let de_input_export =
                                format!("turbo_program:de_command_input/{}/{}", program_id, name);
                            let de_input_extern =
                                format_ident!("de_command_input_{}_{}", program_name, name);

                            // Command registration metadata embedded .turbo_programs
                            let metadata_string = format!("{program_name}/commands/{name},");
                            let metadata_bytes =
                                metadata_string.as_bytes().iter().map(|b| quote! { #b });
                            let metadata_len = metadata_bytes.len();
                            let metadata_ident =
                                format_ident!("command_metadata_{}_{}", program_name, name);
                            ...
                            new_items.extend(syn::parse2::<syn::File>(expanded).unwrap().items);
                            handled = true;
                            break;
                        }
                            ...
                            .map_or(false, |seg| seg.ident == CHANNEL_ATTR_IDENT)
                        {
                            ...
                            let handler_export =
                                format!("turbo_program:channel_handler/{}/{}", program_id, name);
                            let handler_extern =
                                format_ident!("channel_handler_{}_{}", program_name, name);
                            let de_send_export =
                                format!("turbo_program:de_channel_send/{}/{}", program_id, name);
                            let de_send_extern =
                                format_ident!("de_channel_send_{}_{}", program_name, name);
                            let de_recv_export =
                                format!("turbo_program:de_channel_recv/{}/{}", program_id, name);
                            let de_recv_extern =
                                format_ident!("de_channel_recv_{}_{}", program_name, name);
                            let metadata_string = format!("{program_name}/channels/{name},");
                            let metadata_bytes =
                                metadata_string.as_bytes().iter().map(|b| quote! { #b });
                            let metadata_len = metadata_bytes.len();
                            let metadata_ident =
                                format_ident!("channel_metadata_{}_{}", program_name, name);
        ...
        *items = new_items;
        ...
        let program_metadata_ident =
            format_ident!("turbo_os_program_metadata_{}", program_name);
